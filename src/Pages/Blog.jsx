                    ...
                    <div className="grid grid-cols-1 gap-y-10 md:gap-y-10 md:gap-x-5 md:grid-cols-4 lg:grid-cols-3 xl:gap-10">
                        {blogs?.data?.length > 0 &&
                            blogs?.data?.map((blog, index) => (
                                index === 0 ? 
                                    <div className="grid grid-cols-2 col-span-3 gap-2 border border-[#0000001A] rounded-3xl group hover:border-primary md:gap-5 md:border-0 lg:gap-5 md:col-span-4 lg:col-span-3 xl:gap-10" key={blog?.id}>
                                        <div className="img-container overflow-hidden bg-[#F5FDF9] rounded-3xl min-h-65 col-span-2 lg:col-span-1 lg:min-h-75 xl:min-h-85 2xl:min-h-100">
                                            {blog?.image &&
                                                <img src={blog?.image} alt={blog?.title} className="w-full h-full object-cover rounded-3xl group-hover:scale-105" />                                            
                                            }
                                        </div>
                                        <div className="text-left h-full content-center col-span-2 p-6 md:p-0 lg:col-span-1">
                                            {blog?.category?.length > 0 &&
                                                <p className="text-lg font-commissioner font-medium text-primary mb-3 md:mb-3 lg:text-base 2xl:text-lg 2xl:mb-5">
                                                    {blog?.category?.map((cat, ind) => cat + (ind+1 !== blog?.category?.length ? ',' : "") )}
                                                </p>
                                            }
                                            <Link to={`/blog/${blog?.slug}`} className="text-[28px] text-secondary leading-[34px] font-bold hover:text-[#2e988b] md:text-[32px] md:leading-[38px] 2xl:text-[40px] 2xl:leading-[46px]">{blog?.title?.replace(/&amp;/g, "&")}</Link>
                                            <div className="flex gap-2 items-center mt-3 mb-3 lg:mt-4 xl:gap-3 xl:mt-6 2xl:mt-8 2xl:mb-5">
                                                <p className="text-lg text-[#838B9C] font-commissioner 2xl:text-xl">by <span className="text-primary">{blog?.author}</span></p>
                                                <span className="inline-block bg-[#BEC3CE] w-2 h-2 rounded-full  2xl:w-3 2xl:h-3"></span>
                                                <p className="text-lg text-[#838B9C] font-commissioner 2xl:text-xl">{blog?.date}</p>
                                            </div>
                                            <p className="text-base text-[#4B5563] font-commissioner md:text-lg 2xl:text-xl">{blog?.excerpt}</p>
                                        </div>
                                    </div> :
                                <BlogCard data={{title : blog?.title?.replace(/&amp;/g, "&"), slug: blog?.slug, text : blog?.excerpt, image : blog?.image, author : blog?.author, date : blog?.date}} key={blog?.id} />                                    
                            ))
                        }
                    </div>
                    {blogs?.total_pages > 1 &&
                        <div className="pt-8 pb-0 flex gap-2 justify-center items-center md:gap-5 md:pt-8 md:pb-0 xl:py-8 xl:gap-3 2xl:gap-5">
                            <button className={`w-10 h-10 text-center content-center rounded-full md:w-12 md:h-12 2xl:w-15 2xl:h-15 ${blogs?.current_page>1 ? "cursor-pointer text-white bg-primary" : "text-primary/30 bg-primary/10 cursor-not-allowed"}`} disabled={blogs?.current_page<=1} onClick={() => updatePage(page-1)}>
                            ...
                            {Array.from({ length: blogs?.total_pages || 0 }, (_, index) => index + 1)?.map((p) => (
                                <button className={`w-10 h-10 text-center text-lg font-commissioner font-semibold content-center rounded-full cursor-pointer md:w-12 md:h-12 2xl:w-15 2xl:h-15 ${blogs?.current_page === p ? "text-white bg-primary" : "text-primary bg-primary/10"} hover:text-white hover:bg-primary`} key={p} onClick={() => updatePage(p)}>
                                    {p}
                                </button>
                            ))}
                            <button className={`w-10 h-10 text-center content-center rounded-full md:w-12 md:h-12 2xl:w-15 2xl:h-15 ${blogs?.current_page < blogs?.total_pages ? "cursor-pointer text-white bg-primary" : "text-primary/30 bg-primary/10 cursor-not-allowed"}`} disabled={blogs?.current_page>=blogs?.total_pages} onClick={() => updatePage(page+1)}>
                                <svg width="18" height="17" viewBox="0 0 18 17" fill="none" className="mx-auto" xmlns="http://www.w3.org/2000/svg">
